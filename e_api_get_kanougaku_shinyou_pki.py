...
def func_p_sd_date(int_systime):
    # 年.月.日-時:分:秒 の部分を作成
    str_date = int_systime.strftime("%Y.%m.%d-%H:%M:%S")
    
    # マイクロ秒（6桁ゼロ埋め）から先頭の3桁を切り出してミリ秒を作成
    str_micro = f"{int_systime.microsecond:06d}"
...
# 備考: urllib.parseで簡単にURLエンコーディングできます。
# 他言語等に移植する場合に備えて残してあります。
# 
...
def func_kanougaku_shinyou(int_p_no, class_login_property):
    ...
    str_RequestMethod = 'GET'
    str_api_response = func_api_req(str_RequestMethod, str_url)
